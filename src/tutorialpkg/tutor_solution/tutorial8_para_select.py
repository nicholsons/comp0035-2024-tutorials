...
def select_join_groupby(cursor):
    ...
    sql = (
            ...
            'FROM MedalResult '
            'LEFT JOIN Event on MedalResult.event_id = Event.event_id '
...
if __name__ == '__main__':
    ...
    print("\nAll categories from the 'Disability' table in alphabetical order:")
    ...
    print("\nUnique 'region' names from the 'Country' table:")
    [print(row[0]) for row in result]
    ...
    print("\nStart and end dates of all Events that occured in years between 1960 and 1969:")
    [print(row) for row in result]

    # 4. Find 5 country codes from the 'Host' table.
    result = select_limit(cur, table="Host", column="country_code", limit=5)
    print("\n5 country codes from the 'Host' table:")
    [print(row[0]) for row in result]

    # 5. Find the event_id and number of teams in the MedalResult table for each Event.
    result = select_groupby(cursor=cur, table="MedalResult", group_column="event_id", count_column="country_code")
    print("\nThe event_id and number of teams in the MedalResult table for each Event.")
    [print(row) for row in result]
    ...
    print("\nevent_id and number of teams in the MedalResult table for event with event_id 27.")
    [print(row) for row in result]
    ...
    print("\n Event name and number of teams in the MedalResult table for event with event_id 27.")
    [print(row) for row in result]

    # 8. Find the year, host name, number of male participants and number of female participants in all winter games.
    result = select_event_participants_winter(cur)
    print("\n The year, host name, number of male participants and number of female participants in all winter games.")
    [print(row) for row in result]
    ...
    print("\nFind the year, event name, event type and rank where the Faroe Islands appear in the MedalResults.")
    [print(row) for row in result]
    ...
    print("\nFind the events that included the disability category 'Intellectual Disability' and sort alphabetically.")
    [print(row) for row in result]
